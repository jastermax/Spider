...
def spider(listurl):
    ...
    print("完成爬取:"+url.split('_')[-1],start-end)
...
if __name__ == "__main__":
    ...
    lock = threading.Lock()  # 创建线程锁
    thread_1 = threading.Thread(target=spider, args=(listurl[:50],))
    thread_2 = threading.Thread(target=spider, args=(listurl[50:100],))
    thread_3 = threading.Thread(target=spider, args=(listurl[100:150],))
    thread_4 = threading.Thread(target=spider, args=(listurl[150:],))
    thread_1.start()
    thread_2.start()
    thread_3.start()
    thread_4.start()
